...
<div class="containers">
	...
	@foreach($data['vehicle'] as $item)
		<div class="right_vehcile_cell">
			<span class="plate_number">{{$item->plate_number}}</span> <span class="area">从 {{$item->from['province']}} {{$item->from['city']}} {{$item->from['area']}} 到 {{$item->to['province']}} {{$item->to['city']}} {{$item->to['area']}}</span>
			<a href="/user/vehicle/edit/{{$item->id}}">编辑</a>
		</div>
	@endforeach
	</div>
